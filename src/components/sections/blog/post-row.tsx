...
export function PostRow({
  ...
  return (
    <article
      className={cn(
        "group relative grid gap-6 rounded-xl focus-within:ring-2 focus-within:ring-orange-400 focus-within:ring-offset-4 focus-within:ring-offset-surface md:grid-cols-12 md:items-center",
        featured && "md:gap-8",
      )}
    >
      <div
      ...
      >
        <ImageSlot
          ...
          ratio="16/9"
          className="[&_img]:transition-transform [&_img]:duration-500 group-hover:[&_img]:scale-[1.03] motion-reduce:group-hover:[&_img]:scale-100"
        >
          <Photo
            photo={blogCoverPhoto(post.tags, post.slug)}
            treatment="grade"
            priority={featured}
            sizes={
              featured
      ...
      >
        <div className="flex flex-wrap gap-2">
          {featured && <CardBadge nr="Em destaque" />}
        ...
        <span className="mt-4 inline-flex items-center gap-1.5 font-medium text-petrol-500">
          Ler artigo
          <ArrowRight
            aria-hidden
